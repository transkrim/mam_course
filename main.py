...
def math_model(TT, y):
    ...
    result_2.append(y)#288.461538 28.8
    result_3.append(0.0)
...
#         n_counter += 1
#
#     return print(f"Максимальная концентрация винилацетата: {c_max_element}"), \
#            print(f"Искомая длина трубы ректора: {l_target}"), \
#            print(f"Искомая температура: {t_target}")
...
def imitation(Tt):
    time, numbers = lab_6()
    numbers.insert(0, Tt)
    print(f'Оптимальная температура: {440}')
    print(f'Оптимальная длина: {4.8}')
    List_C = list()
    for i in numbers:
        tempC = math_model(440, i)
        List_C.append(tempC)

    plot(time[:502], numbers[:502], List_C[:502])
...
def plot(Time, y, C):
    ...
    axes[1].plot(Time, C, color='black')
    axes[1].grid(True)
    axes[1].set(ylabel="Концентрация(моль/м^3)")
    plt.subplots_adjust(wspace=0.5, hspace=0)
    plt.show()
